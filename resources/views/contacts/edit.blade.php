...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-lg mx-auto">
            ...
            <form action="{{ route('contacts.update', $contact) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('purpose') border-red-500 @enderror"
                        ...
                        <option value="general" {{ old('purpose', $contact->purpose) == 'general' ? 'selected' : '' }}>General Inquiry</option>
                        <option value="support" {{ old('purpose', $contact->purpose) == 'support' ? 'selected' : '' }}>Technical Support</option>
                        <option value="feedback" {{ old('purpose', $contact->purpose) == 'feedback' ? 'selected' : '' }}>Feedback</option>
                        <option value="business" {{ old('purpose', $contact->purpose) == 'business' ? 'selected' : '' }}>Business Opportunity</option>
                        <option value="other" {{ old('purpose', $contact->purpose) == 'other' ? 'selected' : '' }}>Other</option>
                    </select>
                </div>
